...
class Marcel::MimeType::NameTest < ActiveSupport::TestCase
  ...
  extensions.each do |(extension, content_type)|
    test "gets type for #{content_type} given file extension #{extension}" do
      ...
      assert_equal content_type, Marcel::MimeType.for(extension: extension.downcase)
      assert_equal content_type, Marcel::MimeType.for(extension: extension[1..-1])
      assert_equal content_type, Marcel::MimeType.for(extension: extension[1..-1].downcase)
    end
  end
end
